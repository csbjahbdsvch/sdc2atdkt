...
def save_cur_predict_result(dres, q, r, d, t, thr, m, sm, p):
...
def evaluate(model, test_loader, model_name, save_path=""):
    ...
    with torch.no_grad():
        ...
        for data in test_loader:
            ...
            # save predict result
            if save_path != "":
                result = save_cur_predict_result(dres, c, r, cshft, rshft, dcur["historycorrs"], m, sm, y)
                fout.write(result+"\n")
...
def group_fusion(dmerge, model, model_name, fusion_type, fout):
    ...
    hasearly = ["dkvmn", "kqn", "akt", "bakt", "saint", "sakt"]
...
def evaluate_question(model, test_loader, model_name, fusion_type=["early_fusion", "late_fusion"], save_path=""):
    ...
    hasearly = ["dkvmn", "kqn", "akt", "saint", "sakt"]
    if save_path != "":
        fout = open(save_path, "w", encoding="utf8")
        if model_name in hasearly:
